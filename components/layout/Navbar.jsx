...
const Navbar = () => {
  ...
  return (
    <div className="fixed top-0 left-0 w-full z-50  md:mt-5 lg:mt-12">
      <div className="md:max-w-6xl px-2 mx-auto">
        <nav className="md:py-4 py-[0.3rem] px-3 sm:px-6 flex items-center justify-between bg-white rounded-lg shadow-[0_4px_18px_5px_#00000066]">
          <div className="flex items-center  w-32 sm:w-56">
            <Link href="/">
              <Image src={logo} alt="Logo" className="" />
            </Link>
            ...
            >
              {navLinks.map(({ label, href }) => (
                <Link
                  ...
                  href={href}
                  onClick={handleLinkClick}
                  className={`hover:text-[#0088CF] block font-normal hover:text-primary ${
                    pathname === href ? "text-[#0088CF]" : "text-black"
                  }`}
                >
                  {label}
                </Link>
